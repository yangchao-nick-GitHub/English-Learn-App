...
const App: React.FC = () => {
  ...
  const { saveCurrentWord, updateStats } = useProgressManager()
  ...
  useEffect(() => {
    if (words !== undefined) {
      ...
      const dictProgress = storedProgress.dictProgress?.[currentDictId]

      let initialIndex = 0
      if (dictProgress) {
        // 如果保存的章节与当前章节不同，需要切换到正确的章节
        if (dictProgress.currentChapter !== currentChapter) {
          setCurrentChapter(dictProgress.currentChapter)
          // 章节切换后，words会重新加载，这里先返回
          return
        }
        // 恢复到保存的单词位置
        initialIndex = Math.min(dictProgress.currentWordIndex, words.length - 1)
      }
  ...
  }, [words, currentDictId, currentChapter])
  ...
  useEffect(() => {
    ...
    if (state.isFinished && !state.isSavingRecord) {
      ...
      // 更新学习进度
      const totalWords = state.chapterData.words?.length || 0
      updateProgress(currentDictId, currentChapter, totalWords, totalWords)

      // 更新用户统计
      const studyTime = Math.floor(state.timerCount / 60) // 转换为分钟
      const wordsTyped = state.chapterData.words?.length || 0
      const speed = state.correctCount / Math.max(1, state.timerCount / 60) // WPM
      const accuracy = (state.correctCount / Math.max(1, state.correctCount + state.wrongCount)) * 100

      updateStats(studyTime, wordsTyped, speed, accuracy)
    }
  ...
  // 保存当前显示的单词作为学习进度点
  useEffect(() => {
    if (!isReviewMode && state.chapterData.index >= 0 && state.chapterData.words?.length > 0) {
      // 使用词书的总单词数而不是当前章节的单词数
      const totalWordsInDict = currentDictInfo.length
      // 保存当前显示的单词位置
      saveCurrentWord(currentDictId, currentChapter, state.chapterData.index, totalWordsInDict)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
